...
public class CustomBookRepositoryImpl implements CustomBookRepository {
    ...
    @Override
    public Mono<Book> findByIdWithAuthor(String id) {
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(Criteria.where("_id").is(id)),
                Aggregation.lookup("authors", "author.$id", "_id", "authorData"),
                Aggregation.unwind("authorData", true),
                Aggregation.lookup("genres", "genres.$id", "_id", "genresData"),
                Aggregation.project()
                        .and("_id").as("id")
                        .and("title").as("title")
                        .and("authorData").as("author")
                        .and("genresData").as("genres")
        );

        return mongoTemplate.aggregate(aggregation, "books", Book.class)
                .next();
    }
